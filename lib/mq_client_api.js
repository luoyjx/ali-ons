...
// 用于匹配 consumeStats 返回的 json 形式的 key
const correctOffsetTableKeyReg = /(\{"brokerName":"\S{0,30}","queueId":\d+,"topic":"\S{0,30}"\})/g;
...
class MQClientAPI extends RemotingClient {
  ...
  * queryConsumerOffset(brokerAddr, requestHeader, timeoutMillis) {
   ...
   * @param {Object} requestHeader - request header
   * @param {Number} timeoutMillis - timeout in milliseconds
   * @return {Object} - consume stats
   */
  * getConsumeStats(addr, requestHeader, timeoutMillis) {
    requestHeader.consumerGroup = this._buildWithProjectGroup(requestHeader.consumerGroup);
    requestHeader.topic = this._buildWithProjectGroup(requestHeader.topic);

    const request = RemotingCommand.createRequestCommand(RequestCode.GET_CONSUME_STATS, requestHeader);
    const response = yield this.invoke(addr, request, timeoutMillis);
    switch (response.code) {
      case ResponseCode.SUCCESS:
      {
        ...
        if (body) {
          ...
          try {
            consumeStats = JSON2.parse(body.toString());
          } catch (err) {
            // server 返回的 json 格式错误导致 JSON.parse 失败
            if (err.message.indexOf('Unexpected token') !== -1) {
              let bodyStr = body.toString();

              bodyStr = bodyStr.replace(correctOffsetTableKeyReg, function(match) {
                return JSON.stringify(match);
              });

              // 校正 json
              consumeStats = JSON2.parse(bodyStr);
            } else {
              throw err;
            }
          }
      ...
      default:
        this._defalutBrokerHandler(request, response);
        break;
    }
  }
  ...
  * sendMessage(brokerAddr, brokerName, msg, requestHeader, timeoutMillis) {
  ...
  // default broker hanlder
  _defalutBrokerHandler(request, response) {
    const err = new Error(response.remark);
    err.name = 'MQBrokerException';
    err.code = response.code;
    throw err;
  }
