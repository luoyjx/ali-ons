...
class MQProducer extends ClientConfig {
  ...
  constructor(options) {
    ...
    this._mqClient.on('consume_stats', stats => { this.emit('consume_stats', stats); });
  ...
  * send(msg, shardingKey) {
    ...
    for (; times < timesTotal && Date.now() - beginTimestamp < maxTimeout; times++) {
      ...
      if (shardingKey && typeof shardingKey === 'string') {
        messageQueue = topicPublishInfo.selectOneMessageQueueByShardingKey(shardingKey);
        msg.properties.__SHARDINGKEY = shardingKey;
      } else {
        messageQueue = topicPublishInfo.selectOneMessageQueue(lastBrokerName);
