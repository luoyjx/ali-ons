...
class MQClient extends MQClientAPI {
  ...
  /**
   * @property {Number} MQClient#pullConsumeStatsInterval
   */
  get pullConsumeStatsInterval() {
    return this._clientConfig.options.pullConsumeStatsInterval;
  }
  ...
  * init() {
    ...
    if (this._clientConfig.options.consumeStatsEnable) {
      this.startScheduledTask('getConsumeStats', this.pullConsumeStatsInterval);
    }
  }
  ...
  * persistAllConsumerOffset() {
  ...
  /**
   * Get consume stats from server
   * @param {String} topic - topic
   * @param {String} consumeGroup - consume group
   * @return {Object} - consume stats result
   */
  * getConsumeStats(topic, consumeGroup) {
    if (Array.from(this._brokerAddrTable.keys()).length === 0) {
      yield this.updateTopicRouteInfoFromNameServer(this._clientConfig.options.topic);
    }

    const brokerAddrTable = this._brokerAddrTable;

    const requestHeader = {
      consumerGroup: consumeGroup || this._clientConfig.options.consumerGroup,
      topic: topic || this._clientConfig.options.topic,
    };

    // Example of consumeStatsResult
    // {
    //   'broker1': {
    //     tps: 123,
    //     totalDiff: 10,
    //     queuesDiff: [
    //       { queue: '{"brokerName":"broker1","queueId":0,"topic":"test-topic"}',
    //       diff: 4 },
    //       { queue: '{"brokerName":"broker1","queueId":1,"topic":"test-topic"}',
    //       diff: 6 }
    //     ]
    //   },
    //   ...
    // }
    const consumeStatsResult = {};
    ...
    for (const brokerName of brokerAddrTable.keys()) {
      ...
      for (const id in oneTable) {
        const addr = oneTable[id];
        if (addr && addr === oneTable[MixAll.MASTER_ID]) {
          try {
            const consumerStats = yield this.getConsumeStats(addr, requestHeader, 3000);
            debug('fetch from %s consumerStats result:', addr);
            debug('consume TPS : ', consumerStats.consumeTps);
            const offsetTable = new Map(Object.entries(consumerStats.offsetTable));
            const consumeStats = new ConsumeStats(offsetTable, consumerStats.consumeTps);
            debug(`message queue => broker ${brokerName} diff ${consumeStats.computeTotalDiff()}`);
            debug('queues diff: \n');
            debug(consumeStats.getQueuesDiff());
            consumeStatsResult[brokerName] = {
              tps: consumeStats.consumeTps,
              totalDiff: consumeStats.computeTotalDiff(),
              queuesDiff: consumeStats.getQueuesDiff(),
            };
          } catch (err) {
            this.logger.error('[MQClient] Get consume stats from [%s] fail .', addr);
            this.logger.error(err);
          }
        }
      }
