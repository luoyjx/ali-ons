...
class MQClient extends MQClientAPI {
  ...
  * init() {
    ...
    if (this._clientConfig.options.consumeStatsEnable) {
      this.startScheduledTask('getConsumeStatsTask', this.pullConsumeStatsInterval);
    }
  }
  ...
  * getConsumeStatsTask() {
    ...
    const requestHeader = {
      consumerGroup: this._clientConfig.options.consumerGroup,
      topic: this._clientConfig.options.topic,
    };
      ...
      for (const id in oneTable) {
        ...
        if (addr && addr === oneTable[MixAll.MASTER_ID]) {
          try {
            ...
            console.log('fetch from %s consumerStats result:', addr);
            console.log('consume TPS : ', consumerStats.consumeTps);
            ...
            console.log(`message queue => broker ${brokerName} diff ${consumeStats.computeTotalDiff()}`);
            console.log('queues diff: \n');
            console.log(consumeStats.getQueuesDiff());
          } catch (err) {
