...
class MQClient extends MQClientAPI {
  ...
  * persistAllConsumerOffset() {
   ...
   */
  * getConsumeStatsTask(topic, consumeGroup) {
    if (this._brokerAddrTable.size === 0) {
      yield this.updateTopicRouteInfoFromNameServer(topic || this._clientConfig.options.topic);
    }
      ...
      for (const id in oneTable) {
        ...
        if (addr && addr === oneTable[MixAll.MASTER_ID]) {
          try {
            let consumerStats = {};
            try {
              consumerStats = yield this.getConsumeStats(addr, requestHeader, 3000);
            } catch (e) {
              console.log(e);
            }

            if (!consumerStats.offsetTable) continue;
